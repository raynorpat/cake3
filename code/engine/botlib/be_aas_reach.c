...
int AAS_Reachability_Step_Barrier_WaterJump_WalkOffLedge(int area1num, int area2num)
{
	...
	vec3_t          ground_beststart = { 0, 0, 0 }, ground_bestend =
	{
	0, 0, 0}, ground_bestnormal =
	{
	0, 0, 0};
	vec3_t          water_beststart = { 0, 0, 0 }, water_bestend =
	{
	0, 0, 0}, water_bestnormal =
	{
	0, 0, 0};
				...
				if(!trace.startsolid && trace.fraction >= 1.0)
				{
					...
					if(AAS_PointAreaNum(trace.endpos) == area2num)
					{
						...
						if(i >= numareas)
						{
							...
							lreach->traveltime =
								aassettings.rs_startwalkoffledge + fabs(ground_bestdist) * 50 / aassettings.phys_gravity;
...
//      Log_Write("shortest distance between %d and %d is %f\r\n", area1num, area2num, bestdist);
		...
		for(i = 0; i < 3; i++)
		{
			...
			AAS_PredictClientMovement(&move, -1, beststart, PRESENCE_NORMAL, qtrue,
									  velocity, cmdmove, 3, 30, 0.1f, stopevent, 0, qfalse);
		...
		{
			lreach->traveltime =
				aassettings.rs_startjump + VectorDistance(bestend, beststart) * 240 / aassettings.phys_maxwalkvelocity;
		}						//end if
		//
		...
		if(velocity[0] || velocity[1])
		{
			...
			for(i = 0; i < 20; i++)
			{
				AAS_PredictClientMovement(&move, -1, areastart, PRESENCE_NORMAL, qfalse,
										  ...
										  SE_ENTERLAVA | SE_HITGROUNDDAMAGE | SE_TOUCHJUMPPAD | SE_TOUCHTELEPORTER, 0,
...
int AAS_Reachability_Grapple(int area1num, int area2num)
{
	...
	aas_area_t     *area1, *area2;
	aas_lreachability_t *lreach;
	vec3_t          areastart = { 0, 0, 0 }, facecenter, start, end, dir, down =
	{
	0, 0, -1};
	vec_t          *v;
			...
			for(j = 0; j < area->numfaces; j++)
			{
				...
				for(l = 0; l < face2->numedges; l++)
				{
					...
					if(abs(edge1num) == abs(edge2num))
					{
						...
						lreach->traveltype = TRAVEL_WALKOFFLEDGE;
						lreach->traveltime =
							aassettings.rs_startwalkoffledge + fabs(mid[2] - trace.endpos[2]) * 50 / aassettings.phys_gravity;
