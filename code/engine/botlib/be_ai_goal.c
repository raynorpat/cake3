	...
	for(ent = AAS_NextBSPEntity(0); ent; ent = AAS_NextBSPEntity(ent))
	{
		...
		if(spawnflags & 1)
		{
			...
			if(!(AAS_PointContents(origin) & CONTENTS_WATER))
			{
				...
				trace =
					AAS_Trace(origin, ic->iteminfo[i].mins, ic->iteminfo[i].maxs, end, -1, CONTENTS_SOLID | CONTENTS_PLAYERCLIP);
			...
			if(li->entitynum && li->entitynum == ent)
			{
				...
				else
				{
					if(entinfo.origin[0] != li->origin[0] ||
					   entinfo.origin[1] != li->origin[1] || entinfo.origin[2] != li->origin[2])
					{
