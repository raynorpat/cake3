...
static const char s_ascii_encoding[] = "0123456789abcdef" "ghijklmnopqrstuv" "wxyzABCDEFGHIJKL" "MNOPQRSTUVWXYZ[]";
...
*/
	...
	if(!strlen(Cvar_VariableString("sv_leagueName")))
	{
		status = GRankNewGameAsync
			(s_server_context,
			 SV_RankNewGameCBF, NULL, GR_OPT_LEAGUENAME, (void *)(Cvar_VariableString("sv_leagueName")), GR_OPT_END);
	}
	else
	{
...
void SV_RankUserCreate(int index, char *username, char *password, char *email)
{
	...
	status = GRankUserCreateAsync
		(s_ranked_players[index].context,
		 username, password, email, SV_RankUserCBF, (void *)&s_ranked_players[index], GR_OPT_END);
...
void SV_RankUserLogin(int index, char *username, char *password)
{
	...
	status = GRankUserLoginAsync
		(s_ranked_players[index].context, username, password, SV_RankUserCBF, (void *)&s_ranked_players[index], GR_OPT_END);
...
qboolean SV_RankUserValidate(int index, const char *player_id, const char *key, int token_len, int rank, char *name)
{
	...
	if((player_id != NULL) && (key != NULL))
	{
		...
		// then validate
		status = GRankPlayerValidate(s_server_context,
									 ranked_player->player_id,
									 ranked_player->token, token_len, GR_OPT_PLAYERCONTEXT, ranked_player->context, GR_OPT_END);
	}
	else
	{
...
void SV_RankUserLogout(int index)
{
	...
	status = GRankSendReportsAsync
		(s_ranked_players[index].context, 0, SV_RankSendReportsCBF, (void *)&s_ranked_players[index], GR_OPT_END);
	...
	else
	{
		...
		cleanup_status = GRankCleanupAsync
			(s_ranked_players[index].context, 0, SV_RankCleanupCBF, (void *)&s_ranked_players[index], GR_OPT_END);

		if(cleanup_status != GR_STATUS_PENDING)
		{
			SV_RankError("SV_RankUserLogout: Expected "
						 "GR_STATUS_PENDING from GRankCleanupAsync, got %s", SV_RankStatusString(cleanup_status));
			...
			for(i = 0; i < sv_maxclients->value; i++)
			{
				...
				else
				{
					if(s_ranked_players[i].context)
					{
						...
						cleanup_status = GRankCleanupAsync
							(s_ranked_players[i].context, 0, SV_RankCleanupCBF, (void *)&(s_ranked_players[i]), GR_OPT_END);
...
static void SV_RankUserCBF(GR_LOGIN * gr_login, void *cbf_arg)
{
		...
		case GR_STATUS_OK:
			...
			if(join_status != GR_STATUS_PENDING)
			{
				SV_RankError("SV_RankUserCBF: Expected GR_STATUS_PENDING "
							 "from GRankJoinGameAsync, got %s", SV_RankStatusString(join_status));
			}
			break;
	...
	if(ranked_player->final_status != QGR_STATUS_NEW)
	{
		...
		{
			SV_RankError("SV_RankUserCBF: Expected GR_STATUS_PENDING "
						 "from GRankCleanupAsync, got %s", SV_RankStatusString(cleanup_status));
