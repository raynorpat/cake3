...
void WindingPlane (winding_t *w, vec3_t normal, vec_t *dist)
{
	...
	for (i = 0; i < w->numpoints; i++)
	{
		...
		if (D_VectorLength(v1) > 0.5 && D_VectorLength(v2) > 0.5) break;
	} //end for
...
vec_t	WindingArea (winding_t *w)
{
	...
	for (i=2 ; i<w->numpoints ; i++)
	{
		...
		total += 0.5 * D_VectorLength ( cross );
	}
	return total;
}
...
void CheckWinding (winding_t *w)
{
	...
	// check the edge isnt degenerate
		...
		if (D_VectorLength (dir) < ON_EPSILON)
...
winding_t *MergeWindings(winding_t *w1, winding_t *w2, vec3_t planenormal)
{
	...
	for (i = 0; i < w2->numpoints; i++)
	{
		...
		for (j = 0; j < numpoints; j++)
		{
			...
			if (D_VectorLength(sepnormal) < 0.9)
			{
