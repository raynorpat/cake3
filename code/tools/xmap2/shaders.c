	...
	while(1)
	{
		...
		while(1)
		{
			...
			else if(!Q_strncasecmp(token, "xmap_", 5))
			{
				...
				else if(!Q_stricmp(token, "xmap_floodLight"))
				{
					/* get color */
					GetTokenAppend(shaderText, qfalse);
				...
				else if(!Q_stricmp(token, "xmap_nodirty"))
				{
					si->noDirty = qtrue;
				}
				...
				/* xmap_lightmapSampleOffset <value> */
				...
				/* ydnar: xmap_fur <numlayers> <offset> <fade> */
